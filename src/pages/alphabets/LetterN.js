...
import { QuestionN } from "../../components/QuestionBox";
...
const LetterN = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionN.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionN.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionN.length}
                </h1>
                <p>{QuestionN[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionN[currentQuestion].answerOption.map((i) => (
