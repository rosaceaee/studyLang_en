...
import { QuestionX } from "../../components/QuestionBox";
...
const LetterX = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionX.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionX.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionX.length}
                </h1>
                <p>{QuestionX[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionX[currentQuestion].answerOption.map((i) => (
