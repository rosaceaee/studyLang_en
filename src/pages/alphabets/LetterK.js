...
import { QuestionK } from "../../components/QuestionBox";
...
const LetterK = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionK.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionK.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionK.length}
                </h1>
                <p>{QuestionK[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionK[currentQuestion].answerOption.map((i) => (
