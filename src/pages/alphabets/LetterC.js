...
import { QuestionC } from "../../components/QuestionBox";
...
const LetterC = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionC.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionC.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionC.length}
                </h1>
                <p>{QuestionC[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionC[currentQuestion].answerOption.map((i) => (
