...
import { QuestionF } from "../../components/QuestionBox";
...
const LetterF = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionF.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionF.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionF.length}
                </h1>
                <p>{QuestionF[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionF[currentQuestion].answerOption.map((i) => (
