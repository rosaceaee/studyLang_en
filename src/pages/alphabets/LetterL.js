...
import { QuestionL } from "../../components/QuestionBox";
...
const LetterL = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionL.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionL.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionL.length}
                </h1>
                <p>{QuestionL[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionL[currentQuestion].answerOption.map((i) => (
