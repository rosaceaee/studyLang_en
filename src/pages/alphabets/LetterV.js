...
import { QuestionV } from "../../components/QuestionBox";
...
const LetterV = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionV.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionV.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionV.length}
                </h1>
                <p>{QuestionV[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionV[currentQuestion].answerOption.map((i) => (
