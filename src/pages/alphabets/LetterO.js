...
import { QuestionO } from "../../components/QuestionBox";
...
const LetterO = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionO.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionO.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionO.length}
                </h1>
                <p>{QuestionO[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionO[currentQuestion].answerOption.map((i) => (
