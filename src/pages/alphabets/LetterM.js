...
import { QuestionM } from "../../components/QuestionBox";
...
const LetterM = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionM.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionM.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionM.length}
                </h1>
                <p>{QuestionM[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionM[currentQuestion].answerOption.map((i) => (
