...
import { QuestionI } from "../../components/QuestionBox";
...
const LetterI = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionI.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionI.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionI.length}
                </h1>
                <p>{QuestionI[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionI[currentQuestion].answerOption.map((i) => (
