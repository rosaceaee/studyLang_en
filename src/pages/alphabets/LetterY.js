...
import { QuestionY } from "../../components/QuestionBox";
...
const LetterY = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionY.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionY.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionY.length}
                </h1>
                <p>{QuestionY[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionY[currentQuestion].answerOption.map((i) => (
