...
import { QuestionG } from "../../components/QuestionBox";
...
const LetterF = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionG.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionG.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionG.length}
                </h1>
                <p>{QuestionG[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionG[currentQuestion].answerOption.map((i) => (
