...
import { QuestionT } from "../../components/QuestionBox";
...
const LetterT = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionT.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionT.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionT.length}
                </h1>
                <p>{QuestionT[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionT[currentQuestion].answerOption.map((i) => (
