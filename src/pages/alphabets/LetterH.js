...
import { QuestionH } from "../../components/QuestionBox";
...
const LetterH = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionH.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionH.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionH.length}
                </h1>
                <p>{QuestionH[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionH[currentQuestion].answerOption.map((i) => (
