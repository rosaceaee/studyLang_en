...
import { QuestionZ } from "../../components/QuestionBox";
...
const LetterZ = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionZ.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionZ.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionZ.length}
                </h1>
                <p>{QuestionZ[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionZ[currentQuestion].answerOption.map((i) => (
