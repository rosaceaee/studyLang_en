...
import { QuestionJ } from "../../components/QuestionBox";
...
const LetterJ = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionJ.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionJ.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionJ.length}
                </h1>
                <p>{QuestionJ[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionJ[currentQuestion].answerOption.map((i) => (
