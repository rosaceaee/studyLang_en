...
import { QuestionP } from "../../components/QuestionBox";
...
const LetterP = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionP.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionP.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionP.length}
                </h1>
                <p>{QuestionP[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionP[currentQuestion].answerOption.map((i) => (
