...
import { QuestionU } from "../../components/QuestionBox";
...
const LetterU = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionU.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionU.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionU.length}
                </h1>
                <p>{QuestionU[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionU[currentQuestion].answerOption.map((i) => (
