...
import { QuestionQ } from "../../components/QuestionBox";
...
const LetterQ = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionQ.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionQ.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionQ.length}
                </h1>
                <p>{QuestionQ[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionQ[currentQuestion].answerOption.map((i) => (
