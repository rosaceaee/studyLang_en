...
import { QuestionW } from "../../components/QuestionBox";
...
const LetterW = () => {
  ...
  const handleClick = (isCorrect) => {
    ...
    if (nextQuestion < QuestionW.length) {
        ...
        <div className="letter-test-con objective">
          {showScore ? (
            <section className="showScore-section">
              {QuestionW.length}개 중에서 {score}개나 맞추셨네요!
            </section>
          ) : (
            <>
              <section className="question-section">
                <h1 style={{ marginLeft: "auto" }}>
                  문제 {currentQuestion + 1}/{QuestionW.length}
                </h1>
                <p>{QuestionW[currentQuestion].questionText}d</p>
              </section>

              <section className="answer-section">
                {QuestionW[currentQuestion].answerOption.map((i) => (
